...
CollectionSystemManager::CollectionSystemManager(Window* window) : mWindow(window), mIsEditing(false), mCustomCollectionsBundle(NULL)
{
	...
	if (mCollectionSystems.find(defaultEditingCollection) != mCollectionSystems.cend() &&
...
void CollectionSystemManager::createCollectionSystemDeclarations()
{
	...
	for (const auto it : tempSystemDecl)
	{
...
CollectionSystemManager::~CollectionSystemManager()
{
	...
	for (const auto collection : mCollectionSystems)
	{
...
{
	for (const auto collSystemDecl : mCollectionSystemDeclsIndex)
	{
...
void CollectionSystemManager::initCustomCollections()
{
	...
	for (const auto systems : customSystems)
	{
...
void CollectionSystemManager::loadEnabledListFromSettings()
{
	...
	{
		collection.second->isEnabled = (std::find(enabledSystems.cbegin(), enabledSystems.cend(), collection.first) != enabledSystems.cend());
	}
}
...
void CollectionSystemManager::updateSystemsList()
{
	...
	for (const auto system : SystemData::sSystemVector)
	{
		...
		{
			ViewController::get()->getGameListView(system);
		}
	}
...
void CollectionSystemManager::refreshCollectionSystems(FileData* file)
{
	...
	for(const auto & collectionSystem : mCollectionSystems)
	{
...
void CollectionSystemManager::deleteCollectionFiles(FileData* file)
{
	...
	for(const auto & system : mCollectionSystems)
	{
...
std::string CollectionSystemManager::getValidNewCollectionName(std::string inName, int index)
{
	...
	for(const auto system : getSystemsInUse())
	{
		if (system == name)
		{
			if(index > 0) {
...
// Retrieve names of themefolders for systems in configfile
